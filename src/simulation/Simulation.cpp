...
void Simulation::ElementAcquired(int type)
{
	...
	if (!elementsAcquired[type] && elementCount[type] >= 10 && !ignoreElementAcquistion && type!=PT_ARAY && type!=PT_CRAY && type!=PT_DRAY && type!=PT_FRAY && type!=PT_BTRY && type!=PT_WIFI && type!=PT_LIFE && type!=PT_WIRE && type!=PT_PRTI && type!=PT_PRTO && type!=PT_LCRY && type!=PT_GPMP && type!=PT_HSWC && type!=PT_PUMP && type!=PT_DLAY && type!=PT_BHOL && type!=PT_WHOL && type!=PT_VOID && type!=PT_CLNE && type!=PT_BCLN && type!=PT_PCLN && type!=PT_PBCN && type!=PT_PVOD && type!=PT_CONV && type!=PT_STOR && type!=PT_INVS && type!=PT_TSNS && type!=PT_PSNS && type!=PT_VENT && type!=PT_VACU && type!=PT_DTEC && type!=PT_PIPE && type!=PT_PPIP && type!=PT_DCEL && type!=PT_ACEL && type!=PT_GBMB && type!=PT_DMG && type!=PT_RPEL && type!=PT_PSTN && type!=PT_FRME && type!=PT_SPWN && type!=PT_SPWN2 && type!=PT_FIGH && type!=PT_TRON && type!=PT_LOLZ && type!=PT_LOVE && type!=PT_MORT && type!=PT_EQVE) {
